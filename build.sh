...
set_naming() {
    if [ -d "${kernel_dir}/drivers/kernelsu" ]; then
        export zip_name="$kernel_name-KSU-$(TZ="Asia/Dhaka" date +"%d%m%Y-%H%M").zip"
    else
        export zip_name="$kernel_name-$(TZ="Asia/Dhaka" date +"%d%m%Y-%H%M").zip"
    fi
}
set_naming_dynamic() {
    if [ -d "${kernel_dir}/drivers/kernelsu" ]; then
        export zip_name="$kernel_name-DN-KSU-$(TZ="Asia/Dhaka" date +"%d%m%Y-%H%M").zip"
    else
        export zip_name="$kernel_name-DN-$(TZ="Asia/Dhaka" date +"%d%m%Y-%H%M").zip"
    fi
}
